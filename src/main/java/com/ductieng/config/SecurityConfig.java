	...
	public UserDetailsService userDetailsService(UserService userService) {
		return username -> {
			...
			return org.springframework.security.core.userdetails.User.withUsername(u.getUsername())
					.password(u.getPassword()).roles(role).build();
		};
	}
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.csrf(csrf -> csrf.csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse())
				...
				.authorizeHttpRequests(auth -> auth.requestMatchers("/css/**", "/js/**", "/images/**", "/favicon.ico",
						"/", "/laptops/**", "/product/**", "/login", "/register", "/auth/**", // <--- mở quyền cho
																								// /auth/check-unique
						"/error", "/forgot-password", "/reset-password", "/oauth2/**", "/ws-chat/**", "/contact" // <---
																													// thêm
																													// trang
																													// liên
																													// hệ
						...
						.requestMatchers("/api/chat/partners").hasRole("ADMIN").requestMatchers("/api/chat/**")
						.authenticated()
				...
				.oauth2Login(oauth2 -> oauth2.loginPage("/login").defaultSuccessUrl("/laptops", true))
